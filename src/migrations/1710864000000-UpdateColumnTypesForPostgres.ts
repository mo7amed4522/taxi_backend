...
export class UpdateColumnTypesForPostgres1710864000000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "feedback" ALTER COLUMN "score" TYPE smallint`,
    );

    // Change fleet.commissionSharePercent from tinyint to smallint
    await queryRunner.query(
      `ALTER TABLE "fleet" ALTER COLUMN "commissionSharePercent" TYPE smallint`,
    );

    // Change service.prepayPercent from tinyint to smallint
    await queryRunner.query(
      `ALTER TABLE "service" ALTER COLUMN "prepayPercent" TYPE smallint`,
    );

    // Change service.providerSharePercent from tinyint to smallint
    await queryRunner.query(
      `ALTER TABLE "service" ALTER COLUMN "providerSharePercent" TYPE smallint`,
    );

    // Change media.base64 from longtext to text
    await queryRunner.query(
      `ALTER TABLE "media" ALTER COLUMN "base64" TYPE text`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "media" ALTER COLUMN "base64" TYPE longtext`,
    );

    // Revert service.providerSharePercent to tinyint
    await queryRunner.query(
      `ALTER TABLE "service" ALTER COLUMN "providerSharePercent" TYPE tinyint`,
    );

    // Revert service.prepayPercent to tinyint
    await queryRunner.query(
      `ALTER TABLE "service" ALTER COLUMN "prepayPercent" TYPE tinyint`,
    );

    // Revert fleet.commissionSharePercent to tinyint
    await queryRunner.query(
      `ALTER TABLE "fleet" ALTER COLUMN "commissionSharePercent" TYPE tinyint`,
    );

    // Revert feedback.score to tinyint
    await queryRunner.query(
      `ALTER TABLE "feedback" ALTER COLUMN "score" TYPE tinyint`,
    );

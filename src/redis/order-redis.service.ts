...
export class OrderRedisService {
    ...
    async getForDriver(driverId: number, distance?: number): Promise<Array<string>> {
        ...
        const _requests = await this.redisService.zrangebyscore(RedisKeys.RequestTime, min, max) as string[];
        const intersection = (requestIds as string[]).filter(x => _requests.includes(x));
        return intersection;

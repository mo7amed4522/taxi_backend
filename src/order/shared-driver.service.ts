...
export class SharedDriverService {
    ...
    async findById(id: number): Promise<DriverEntity> {
        return this.driverRepo.findOneOrFail({ where: { id }, withDeleted: true });
    }
    ...
    async getMaxRadiusForDriverServices(driverId: number): Promise<number> {
        const driver = await this.driverRepo.findOne({ where: { id: driverId }, relations: ['enabledServices'] });
    ...
    async rechargeWallet(transaction: Pick<DriverTransactionEntity, 'status' | 'action' | 'rechargeType' | 'deductType' | 'amount' | 'currency' | 'driverId' | 'requestId' | 'operatorId' | 'paymentGatewayId' | 'refrenceNumber' | 'description'>) {
        let wallet = await this.driverWalletRepo.findOne({ where: { driverId: transaction.driverId, currency: transaction.currency } });
        ...
        if (wallet == null) {
            ...
            await this.driverWalletRepo.update(wallet.id, { balance: parseFloat(transaction.amount.toString()) + parseFloat(wallet.balance.toString()) });

...
export class SharedOrderService {
    ...
    async calculateFare(input: { points: Point[], twoWay?: boolean, coupon?: CouponEntity, riderId?: number }) {
        ...
        const feeMultiplier = (await this.sharedFleetService.getFleetById(fleetIdsInPoint[0]))?.feeMultiplier ?? 1;
        const _cats = cats.map(cat => {
            ...
            const _services = services.filter(x => servicesInRegion.filter(y => y.id == x.id).length > 0).map(service => {
                const cost = this.servicesService.calculateCost(service, metrics.distance, metrics.duration, new Date(), feeMultiplier, isResident);
    ...
    async createOrder(input: { riderId: number, serviceId: number, intervalMinutes: number, points: Point[], addresses: string[], operatorId?: number, twoWay?: boolean, optionIds?: string[], couponCode?: string }): Promise<RequestEntity> {
        ...
        const feeMultiplier = (await this.sharedFleetService.getFleetById(fleetIdsInPoint[0]))?.feeMultiplier ?? 1;
        const cost = this.servicesService.calculateCost(service, metrics.distance, metrics.duration, eta, feeMultiplier, isResident) + optionFee;
        const regions = await this.regionService.getRegionWithPoint(input.points[0]);
    ...
    async processPrePay(orderId: number) {
        let order: RequestEntity = await this.orderRepository.findOneOrFail({ where: { id: orderId }, relations: ['service', 'driver', 'driver.fleet', 'rider'] });
        ...
        this.pubSub.publish('orderCreated', { orderCreated: order, driverIds: driversWithService.map(driver => driver.id) });
        this.driverNotificationService.requests(driversWithService);
        return this.orderRepository.findOneOrFail({ where: { id: orderId }, relations: ['service', 'driver', 'driver.fleet', 'rider'] });
    }

    async finish(orderId: number, cashAmount = 0.0) {
        const order: RequestEntity = await this.orderRepository.findOneOrFail({ where: { id: orderId }, relations: ['service', 'driver', 'driver.fleet', 'rider'] });
    ...
    async assignOrderToDriver(orderId: number, driverId: number) {
        const [travel, driverLocation] = await Promise.all([
            this.orderRepository.findOne({ where: { id: orderId }, relations: ['driver', 'driver.car', 'driver.carColor', 'service'] }),
        ...
        const metrics = driverLocation != null ? await this.googleServices.getSumDistanceAndDuration([travel.points[0], driverLocation as Point]) : { distance: 0, duration: 0 };
        const dt = new Date();
        ...
        await this.orderRepository.update(orderId, { status: OrderStatus.DriverAccepted, etaPickup: new Date(etaPickup), driverId });
        const result = await this.orderRepository.findOneOrFail({ where: { id: orderId }, relations: ['driver', 'driver.car', 'driver.carColor', 'service', 'rider'] });
